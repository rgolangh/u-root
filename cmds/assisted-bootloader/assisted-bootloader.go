...
var (
    ...
    token      = flag.String("token", "", "The bearer token for the authorizing the api call")
    infraEnvID = flag.String("infraEnvID", "", "The ID of the infraenv object")
)

func main() {
    flag.Parse()
    if *token == "" || *infraEnvID == "" {
        log.Fatalf("token or infraEnvID are empty. Pass -token=\"ABC\" and -inrfaEnvID=\"XYZ\"")
    }
    ...
    f := fmt.Sprintf("%s/assisted-install/v2/infra-envs/%s/downloads/files?file_name=ipxe-script", *apiURL, *infraEnvID)
    os.Setenv("CURL_GET_HDR_Authorization", fmt.Sprintf("Bearer %s", *token))

    var images []boot.OSImage

    var l dhclient.Lease
    l, err = newManualLease(f, filteredIfs[0])
    if err != nil {
        log.Fatal(err)
    }
