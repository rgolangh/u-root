...
func Test_parser_parseIpxe(t *testing.T) {
    type fields struct {
    ...
    tests := []struct {
        ...
        {
            name: "ignore_initrd_name_arg",
            fields: fields{
                bootImage: nil,
                wd:        nil,
                log:       nil,
                schemes:   curl.DefaultSchemes,
            },
            args: args{
                config: `#!ipxe
initrd --name https://foo.bar.baz/foo?bar&baz==foo
kernel https://foo.bar.baz/kernel`,
            },
            wantErr: false,
        },
    }
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            c := &parser{
                bootImage: tt.fields.bootImage,
                wd:        tt.fields.wd,
                log:       tt.fields.log,
                schemes:   tt.fields.schemes,
            }
            if err := c.parseIpxe(tt.args.config); (err != nil) != tt.wantErr {
                t.Errorf("parseIpxe() error = %v, wantErr %v", err, tt.wantErr)
            }
            fmt.Printf("config %+v", c.bootImage)
        })
    }
}
